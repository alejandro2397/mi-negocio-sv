...
@Composable fun App() {
    val context=androidx.compose.ui.platform.LocalContext.current; val c=remember{AppContainer(context)}; val repo=remember{BusinessRepository(c.productDao,c.saleDao)}
...
@Composable fun SaleScreen(products:List<Product>,sell:(Product,Double)->Unit){var selected by remember{mutableStateOf<Product?>(null)};var q by remember{mutableStateOf("1")};Column(Modifier.padding(20.dp)){Text("Nueva venta",style=MaterialTheme.typography.headlineMedium);Spacer(Modifier.height(12.dp));products.forEach{p->Button(Modifier.fillMaxWidth().padding(vertical=3.dp),{selected=p}){Text("${p.name} • $%.2f • Stock %.2f".format(p.price,p.stock))}};selected?.let{p->Spacer(Modifier.height(10.dp));Text("Producto: ${p.name}");TextField(q,{q=it},label={Text("Cantidad")});Spacer(Modifier.height(8.dp));Button(q.toDoubleOrNull()!=null&&q.toDouble()>0&&q.toDouble()<=p.stock,{sell(p,q.toDouble());selected=null;q="1"}){Text("Registrar venta")}}}}
