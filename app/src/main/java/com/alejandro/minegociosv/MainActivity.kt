...
@Composable fun App() {
    ...
    Scaffold(bottomBar={NavigationBar{listOf("Inicio","Productos","Ventas","Escanear").forEach{s->NavigationBarItem(screen==s,{screen=s},{},{Text(s)})}}}){p->Box(Modifier.padding(p).fillMaxSize()){when(screen){"Productos"->ProductScreen(products){n,cost,price,stock->scope.launch{repo.addProduct(n,cost,price,stock)}};"Ventas"->SaleScreen(products,sales){product,q->scope.launch{repo.addSale(product,q)}};"Escanear"->InvoiceScanScreen();else->Home(products,sales)}}}
}
